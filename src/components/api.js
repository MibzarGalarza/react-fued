...
export function Api() {
  ...
  useEffect(() => {
    function fetchEmployeeData() {
      fetch('http://localhost/xml-2/xml.php') // Cambia la URL a la correcta
        .then(response => {
          if (!response.ok) {
                  ...
                  {targetData ? (
                    targetData.map((target, index) => (
                      <div key={index} className="card">
                        <p className="card-title">{target.cve_municipio}</p>
                        <p className="card-subtitle">{target.desc_municipio}</p>
                        <p className="card-description">{target.id_indicador}</p>
                        <p className="card-date">{target.indicador}</p>
                        <p className="card-date">{target.año}</p>
                        <p className="card-date">{target.valor}</p>
                        <p className="card-date">{target.unidad_medida}</p>
